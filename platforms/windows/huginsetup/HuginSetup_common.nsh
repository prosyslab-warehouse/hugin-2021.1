...
;General  
  ...
  OutFile "Hugin++Setup_${HUGIN_VERSION}-${HUGIN_VERSION_BUILD}-Windows-${ARCH_TYPE}bit.exe"
...
;Interface Configuration
  ...
  !define MUI_FINISHPAGE_LINK_LOCATION "https://sourceforge.net/projects/huginplusplus/"
...
Section "!Hugin++ ${HUGIN_VERSION}-${HUGIN_VERSION_BUILD}" SecHugin
  ...
  WriteRegStr HKCR ".pto" "" "Hugin++Project"
  WriteRegStr HKCR "Hugin++Project" "" "Hugin++ PTO"
  WriteRegStr HKCR "Hugin++Project\DefaultIcon" "" "$INSTDIR\bin\hugin.exe,0"
  WriteRegStr HKCR "Hugin++Project\shell\open\command" "" '"$INSTDIR\bin\hugin.exe" "%1"'
...
SectionGroup /e $(TEXT_SecShortcuts) SecShortcuts
  Section $(TEXT_SecShortcutPrograms) SecShortcutPrograms
    ...
    CreateShortCut "$SMPROGRAMS\${APP_NAME}\Uninstall.lnk" "${UNINST_EXE}" "" "$INSTDIR\share\hugin\xrc\data\hugin.ico"
    CreateShortCut "$SMPROGRAMS\${APP_NAME}\Hugin++.lnk" "$INSTDIR\bin\hugin.exe" "" "$INSTDIR\share\hugin\xrc\data\hugin.ico"
    CreateShortCut "$SMPROGRAMS\${APP_NAME}\Calibrate Lens GUI.lnk" "$INSTDIR\bin\calibrate_lens_gui.exe" "" "$INSTDIR\share\hugin\xrc\data\hugin.ico"
    CreateShortCut "$SMPROGRAMS\${APP_NAME}\Batch Processor.lnk" "$INSTDIR\bin\PTBatcherGUI.exe" "" "$INSTDIR\share\hugin\xrc\data\hugin.ico"
  SectionEnd
  
  Section $(TEXT_SecShortcutDesktop) SecShortcutDesktop
    AddSize 1
    SetOutPath "$INSTDIR\bin"
    CreateShortCut "$DESKTOP\Hugin++.lnk" "$INSTDIR\bin\hugin.exe" "" "$INSTDIR\share\hugin\xrc\data\hugin.ico"
  SectionEnd
SectionGroupEnd
...
Section "un.Uninstall Hugin++"
  ...
  Delete "$SMPROGRAMS\${APP_NAME}\Calibrate Lens GUI.lnk"
  Delete "$SMPROGRAMS\${APP_NAME}\Batch Processor.lnk"
  RMDir "$SMPROGRAMS\${APP_NAME}"

  !insertmacro UNINSTALL.LOG_END_UNINSTALL

  DeleteRegKey HKCR "Hugin++Project\shell\open\command"
  DeleteRegKey HKCR ".pto"
  DeleteRegKey HKCR "Hugin++Project"
  ...
  DeleteRegKey HKCR "SOFTWARE\Classes\.pto"
  DeleteRegKey HKCR "SOFTWARE\Classes\Hugin++Project"
SectionEnd
