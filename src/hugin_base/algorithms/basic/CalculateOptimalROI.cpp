...
bool CalculateOptimalROI::calcOptimalROI(PanoramaData& panorama)
{
    ...
    try
    {
        testedPixels.resize(((int64_t)o_optimalSize.x) * ((int64_t)o_optimalSize.y), false);
        pixels.resize(((int64_t)o_optimalSize.x) * ((int64_t)o_optimalSize.y), false);
    }
    catch(std::bad_alloc&)
    {
...
{
    if(!testedPixels[((int64_t)j) * ((int64_t)o_optimalSize.x) + ((int64_t)i)])
    {
        bool inside;
        ...
        testedPixels[((int64_t)j) * ((int64_t)o_optimalSize.x) + ((int64_t)i)] = true;
        pixels[((int64_t)j) * ((int64_t)o_optimalSize.x) + ((int64_t)i)] = inside;
    ...
    {
        return pixels[((int64_t)j) * ((int64_t)o_optimalSize.x) + ((int64_t)i)];
    }
}
